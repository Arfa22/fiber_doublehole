...
// Generate random positions within the 20 cm x 20 cm area
      xPos = (2.0 * G4UniformRand() - 1.0) * 10.0 * cm;  // Random x position within the square (±10 cm)
      yPos = (2.0 * G4UniformRand() - 1.0) * 10.0 * cm;  // Random y position within the square (±10 cm)
      zPos = 0.0 * cm;  // Fixed z-position (incident on the surface of the scintillator)

      fParticleGun->SetParticlePosition(G4ThreeVector(xPos, yPos, zPos));
      
      // Print the photon position (x, y, z)
      ...
      //fParticleGun->SetParticleMomentumDirection(G4ThreeVector(0., -1, 0.));
       ...
       // Print the photon direction (xVec, yVec, zVec)
            G4cout << "Photon Direction (xVec, yVec, zVec): "
                   << xVec << ", " << yVec << ", " << zVec << G4endl;
