...
void FPEventAction::AddPhotonData(const G4ThreeVector& position, const G4ThreeVector& direction) {
  photonPositions.push_back(position);
  photonDirections.push_back(direction);
}
...
void FPEventAction::EndOfEventAction(const G4Event* evt)
{
  ...
  std::map<G4int,G4int*>::iterator itr;#include "G4ThreeVector.hh"
